function valid() {
  ...
  var accomodation = document.getElementsByName('accomodation')[0].value
...
function valmobo(mobo) {
  var numbers = /^[0-9]+$/
  if (mobo.length == 10) {
    if (mobo.match(numbers)) return true
    else {
    ...
    if (search.length == 1) {
      $_ul.find('label').each(function () {
        if ($(this).text().toLowerCase().charAt(0) == search.charAt(0)) {
          ...
          if ($_not_found.hasClass('hide')) $_not_found.removeClass('hide')
        }
      })
    } else {
